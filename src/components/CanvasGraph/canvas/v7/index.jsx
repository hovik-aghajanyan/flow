...
export default function CanvasGraph() {
    ...
    useEffect(() => {
        if (animationProgress < 1) {
            const frame = requestAnimationFrame(() => {
                const np = Math.min(animationProgress + ANIMATION_STEP, 1);
                setAnimationProgress(np);
                setPositions(layoutNodes(np));
                if (np === 1) setAnimatingNodes([]);
            });
            return () => cancelAnimationFrame(frame);
        }
    }, [animationProgress, animatingNodes]);
    ...
    useEffect(() => {
        ...
        const mx = lastMouse.current.x - rect.left - offset.x;
        const my = lastMouse.current.y - rect.top - offset.y;
        let found = null;
    ...
    function drawNode(ctx, { x, y, label, isSelected }, r) {
        ...
        for (let i = 1; i < words.length; i++) {
            ...
            if (ctx.measureText(test).width > r * 1.8) {
                lines.push(line);
                line = words[i];
            } else {
                line = test;
    ...
    function drawLine(ctx, from, to, hl = false) {
        ...
        ctx.moveTo(sx, sy);
        ctx.lineTo(ex, ey);
        ctx.strokeStyle = hl ? "#e67e22" : "#555";
        ctx.lineWidth = hl ? 3 : 2;
        ctx.stroke();
        const sz = 8;
        ctx.beginPath();
        ctx.fillStyle = hl ? "#e67e22" : "#555";
        ctx.moveTo(ex, ey);
    ...
    return (
        ...
        >
            ...
            <canvas
                ...
                onClick={handleClick}
                style={{
                    cursor: hoveredNode ? "pointer" : isDragging.current ? "grabbing" : "grab",
                    background: "#fff",
                }}
            />
        </div>
